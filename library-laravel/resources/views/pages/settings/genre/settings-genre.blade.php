...
@endif
    ...
    <div class="height-kategorije pb-[30px] scroll">
        <div class="flex items-center justify-between px-[30px] py-4 space-x-3 rounded-lg">
            <a href="{{ route('new-genre') }}" class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] rounded hover:bg-[#4558BE]">
            ...
            <div class="flex items-center">
                <div class="relative text-gray-600 focus-within:text-gray-400">
                    ...
                    <form action="{{route('search-genre')}}" method="post" id="categorySearch">
                        @csrf
                        @method('POST')
                        <input type="search" name="search_genre"
                    ...
                    @foreach($query as $data)
                        <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                            ...
                            <td class="flex flex-row items-center px-4 py-4">
                                <img
                                    style="height:30px"
                                    src="{{$data->default == 'false' ? '/storage/settings/genre/' . $data->icon : $data->icon}}"
                                    alt="{{$data->name}}"
                                    title="{{$data->name}}">
                                <p class="ml-4 text-center no-select">{{$data->name}}</p>
                            </td>
                            <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                                ...
                                <div
                                    ...
                                    <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                        ...
                                        <div class="py-1">
                                            <a href="{{route('edit-genre', $data->id)}}" tabindex="0"
                                            ...
                                            <form method="POST" action="{{route('destroy-genre', $data->id)}}">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    tabindex="0"
                        ...
                        <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                            ...
                            <div
                                ...
                                <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                    aria-labelledby="headlessui-menu-button-1" id="headlessui-menu-items-117" role="menu">
                                    <div class="py-1">
                                        <a href="{{route('edit-genre', $genre->id)}}" tabindex="0"
                                            class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                            role="menuitem">
                                            <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                            <span class="px-4 py-0">Izmijeni žanr</span>
                                        ...
                                        <form method="POST" action="{{route('destroy-genre', $genre->id)}}">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                            tabindex="0"
