...
<div id="hideDiv" class="flex p-2 mt-2 mb-1 text-sm text-green-700 bg-green-200 rounded-lg dark:bg-green-200 dark:text-green-800" role="alert">
    ...
    <div>
      <span class="font-medium">Success!</span> {{session('success-binding')}}
    </div>
  </div>
@endif

{{-- Session message for binding delete --}}
@if (session()->has('binding-deleted'))
<div id="hideDiv" class="flex p-2 mt-2 mb-1 text-sm text-red-700 bg-red-200 rounded-lg dark:bg-red-200 dark:text-red-800" role="alert">
    ...
    <span class="sr-only">Info</span>
    <div>
      <span class="font-medium">Success!</span> {{session('binding-deleted')}}
    </div>
  </div>
@endif
                </h1>
    ...
    <div class="height-kategorije pb-[30px] scroll">
        <div class="flex items-center justify-between px-[30px] py-4 space-x-3 rounded-lg">
            <a href="{{ route('new-binding') }}" class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] rounded hover:bg-[#4558BE]">
            ...
            <div class="flex items-center">
                <div class="relative text-gray-600 focus-within:text-gray-400">
                    ...
                    <form action="{{route('search-binding')}}" method="post" >
                        @csrf
                        @method('POST')
                        <input type="search" name="search_binding"
                            ...
                            <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                                ...
                                <div
                                    ...
                                    <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                        ...
                                        <div class="py-1">
                                            <a href="{{route('edit-binding', $data->id)}}" tabindex="0"
                                            ...
                                            <form method="POST" action="{{route('destroy-binding', $data->id)}}">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    tabindex="0"
                        ...
                        <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                            ...
                            <div
                                ...
                                <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                    aria-labelledby="headlessui-menu-button-1" id="headlessui-menu-items-117" role="menu">
                                    <div class="py-1">
                                        <a href="{{route('edit-binding', $binding->id)}}" tabindex="0"
                                            class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                            role="menuitem">
                                            <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                            <span class="px-4 py-0">Izmijeni povez</span>
                                        ...
                                        <form method="POST" action="{{route('destroy-binding', $binding->id)}}">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                            tabindex="0"
