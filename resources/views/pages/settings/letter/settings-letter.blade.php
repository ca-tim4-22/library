...
<!-- Title -->
<title>{{__('Podešavanja | Pisma - Online biblioteka')}}</title>
...
<div style="display:none;" id="myDiv">
    <div class="height-kategorije pb-[30px] scroll">
        ...
        <div class="flex items-center justify-between px-[50px] py-8 space-x-3 rounded-lg">
            ...
            class="btn-animation inline-flex items-center text-sm py-2.5 px-5 transition duration-300 ease-in rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE]">
                <i class="fas fa-plus mr-[15px]"></i> {{__('Novo pismo')}}
            </a>
            <div class="flex items-center">
                ...
                <script>
                    document.getElementById('pagination').onchange = function() {
                        window.location = "{{ $letters->url(1) }}&items=" + this.value;
                    };
                </script>
                
                <form method="GET" action="{{ route('setting-letter') }}">
        ...
        <div class="inline-block min-w-full px-[50px] pt-3 align-middle bg-white rounded-bl-lg rounded-br-lg shadow-dashboard">
            ...
            <button type="submit" class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE] button delete-all-letters" data-url="">
                <div class="icon">
                    <svg class="top">
                        <use xlink:href="#top">
                    </svg>
        ...
        e.preventDefault();
                    ...
                    <tr class="border-b-[1px] border-[#e4dfdf]">
                        ...
                        <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                            ...
                            style="outline: none;border: none;font-weight: bold;"
                                ...
                                <span style="padding-top: 1px;">{{__('Izmijeni pismo')}}</span>
                            </button>
                        </td>
                        ...
                        <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                            <button
                            style="outline: none;border: none;font-weight: bold;"
                            class="flex w-full px-1 text-sm leading-5 text-left text-blue-600 outline-none delete-all-letters" 
                            role="menuitem"
                            type="submit"
                            data-url="">
                            <i class="fas fa-trash mr-[5px] ml-[5px] py-1"></i>
                            <span style="padding-top: 1px;">{{__('Izbriši pismo')}}</span>
                            </button>
                            </td>
                        ...
                        <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                            ...
                            <div
                                ...
                                <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                    ...
                                    <div class="py-1">
                                        <a href="{{route('edit-letter', $letter->name)}}" tabindex="0"
                                            class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                            role="menuitem">
                                            <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                            <span class="px-4 py-0">{{__('Izmijeni pismo')}}</span>
                                        </a>
                                    
                                        <button 
                                        data-id="{{ $letter->id }}" 
                                        data-action="{{ route('letters.destroy', $letter->id) }}"onclick="deleteConfirmation({{$letter->id}})"
            ...
            <div class="m-3">{!! $letters->links() !!}</div>
    ...
    @else 
        ...
        <div class="w-[400px] flex items-center px-6 py-4 my-4 text-lg bg-[#3f51b5] rounded-lg">                       
            ...
            <p class="font-medium text-white">{{__('Trenutno nema pisama u bazi podataka!')}}</p>
        </div>
...
<script type="text/javascript">
    function deleteConfirmation(id) {
        swal({
            title: "Izbriši?",
            text: "Da li ste sigurni da želite da izbrišete pismo?",
            type: "warning",
            showCancelButton: !0,
            timer: '5000',
            animation: true,
            ...
            if (e.value === true) {
                ...
                swal(
                    'Uspješno!',
                    'Pismo je uspješno izbrisano.',
                    'success'
                    ).then(function() {
                     window.location.href = "/podesavanja/pisma";
                    });
                $.ajax({
                    type: 'POST',
                    url: "{{url('/podesavanja/pisma')}}/" + id,
...
// Script for edit letter profile top header
function editLetter() {
var name = $('#check:checked').val();
window.location.href = "/podesavanja/izmijeni-pismo/" + name;
}
</script>
...
<script type="text/javascript">
   ...
   $('.delete-all-letters').on('click', function(e) {
   var idsArr = [];  
   ...
   swal({
    title: "Greška!",
    text: "Morate selektovati makar jedno pismo.",
    type: "error",
    timer: 1500,
