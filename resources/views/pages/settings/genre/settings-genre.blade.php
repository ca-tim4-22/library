...
<!-- Title -->
<title>{{__('Podešavanja | Žanrovi - Online biblioteka')}}</title>
...
<section class="w-screen h-screen pl-[80px] py-4 text-gray-700">
    ...
    <div class="heading mt-[7px]">
        <div class="heading mt-[7px]" style="margin-top: 10px">
            <h1 style="font-size: 30px"
                ...
                {{__('Podešavanja')}}
                {{-- Session message for genre create --}}
                @if (session()->has('success-genre'))
                <script>
                    swal({
                        title: "@lang('Uspješno!')",
    ...
    <div style="display:none;" id="myDiv">
            ...
            <div class="flex items-center justify-between px-[50px] py-8 space-x-3 rounded-lg">
                <a href="{{route('new-genre')}}"
                   class="btn-animation inline-flex items-center text-sm py-2.5 px-5 transition duration-300 ease-in rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE]">
                    <i class="fas fa-plus mr-[15px]"></i> {{__('Novi žanr')}}
                </a>
                <div class="flex items-center">
                    ...
                    <script>
                        document.getElementById('pagination').onchange = function () {
                            window.location = "{{ $genres->url(1) }}&items=" + this.value;
                        };
                    </script>

                    <form method="GET" action="{{ route('setting-genre') }}">
        ...
        <div class="inline-block min-w-full px-[50px] pt-3 align-middle bg-white rounded-bl-lg rounded-br-lg shadow-dashboard">
            ...
            <button type="submit"
                    class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE] button delete-all-genres"
                    data-url="">
                <div class="icon">
                    <svg class="top">
                        <use xlink:href="#top">
            ...
            <table id="sort"
                ...
                <tr class="border-b-[1px] border-[#e4dfdf]">
                    ...
                    <td style="width: 200px"
                        ...
                        <button
                            ...
                            <span style="padding-top: 1px;">{{__('Izmijeni žanr')}}</span>
                        </button>
                    </td>

                    <th style="width: 300px"
                        class="px-4 py-4 tracking-wider text-left checkme"
                        id="arrow">
                        {{__('Naziv žanra')}}
                    </th>
                    ...
                    <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                        <button
                            ...
                            <span style="padding-top: 1px;">{{__('Izbriši žanr')}}</span>
                        </button>
                    </td>

                    <th class="px-4 py-4  tracking-wider text-left checkme"
                        id="arrow">
                        {{__('Opis žanra')}}
                    </th>
                ...
                <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                    ...
                    <td style="width: 300px"
                        class="flex flex-row items-center px-4 py-4">
                        <img
                                style="height:30px"
                                src="{{$genre->default == 'false' ? '/storage/settings/genre/' . $genre->icon : $genre->icon}}"
                                alt="{{$genre->name}}"
                                title="{{$genre->name}}">
                        <p class="ml-4 text-center no-select">
                            {{$genre->name}}</p>
                    </td>
                    ...
                    <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                        ...
                        <div
                            ...
                            <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                ...
                                <div class="py-1">
                                    <a href="{{route('edit-genre', $genre->name)}}"
                                       tabindex="0"
                                       class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                       role="menuitem">
                                        <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                        <span class="px-4 py-0">{{__('Izmijeni žanr')}}</span>
                                    </a>
                                    <button
                                            data-id="{{ $genre->id }}"
                                            data-action="{{ route('genres.destroy', $genre->id) }}"
                                            onclick="deleteConfirmation({{$genre->id}})"
            ...
            <div class="m-3">{!! $genres->links() !!}</div>
    ...
    <div class="w-[400px] flex items-center px-6 py-4 my-4 text-lg bg-[#3f51b5] rounded-lg">
        ...
        <p class="font-medium text-white">{{__('Trenutno nema žanrova u bazi podataka!')}}</p>
    </div>
...
<script type="text/javascript">
    function deleteConfirmation(id) {
        swal({
            ...
            if (e.value === true) {
                ...
                swal(
                    'Uspješno!',
                    'Žanr je uspješno izbrisan.',
                    'success'
                ).then(function () {
                    window.location.href = "/podesavanja/zanrovi";
                });
                $.ajax({
                    type: 'POST',
                    url: "{{url('/podesavanja/zanrovi')}}/" + id,
    ...
    // Script for edit genre profile top header
    function editGenre() {
        var name = $('#check:checked').val();
        window.location.href = "/podesavanja/izmijeni-zanr/" + name;
    }
</script>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.delete-all-genres').on('click', function (e) {
            var idsArr = [];
