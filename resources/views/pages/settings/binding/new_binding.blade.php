...
<!-- Title -->
<title>{{__('Novi povez | Online biblioteka')}}</title>
...
<section class="w-screen h-screen pl-[80px] pb-4 text-gray-700">
    ...
    <div class="scroll height-content section-content">
        <form class="text-gray-700" method="POST"
              action="{{route('store-binding')}}">
            @csrf
            @honeypot
            <div class="flex flex-row ml-[30px]">
                <div class="w-[50%] mb-[150px]">
                    <div class="mt-[20px]">
                        <p>{{__('Naziv poveza')}} <span class="text-red-500">* @error('name') {{$message}} @enderror</span>
                        </p>
                        <input type="text" name="name" id="name"
                               class="flex w-[90%] mt-2 px-2 py-2 text-base bg-white border border-gray-300 shadow-sm appearance-none focus:outline-none focus:ring-2 focus:ring-[#576cdf]"
                               value="{{old('name')}}"
                               onkeydown="clearErrorsNazivPovez()"/>
                    </div>
                </div>
            </div>
