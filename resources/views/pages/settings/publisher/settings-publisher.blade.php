...
<!-- Title -->
<title>Podešavanja | Izdavač - Online Biblioteka</title>
...
{{-- Session message for publisher create --}}
@if (session()->has('success-publisher'))
<div id="hideDiv" class="flex p-2 mt-2 mb-1 text-sm text-green-700 bg-green-200 rounded-lg dark:bg-green-200 dark:text-green-800" role="alert">
    ...
    <div>
      <span class="font-medium">Uspješno!</span> {{session('success-publisher')}}
    </div>
  </div>
@endif

{{-- Session message for publisher delete --}}
@if (session()->has('publisher-deleted'))
<div id="hideDiv" class="flex p-2 mt-2 mb-1 text-sm text-red-700 bg-red-200 rounded-lg dark:bg-red-200 dark:text-red-800" role="alert">
    ...
    <span class="sr-only">Info</span>
    <div>
      <span class="font-medium">Uspješno!</span> {{session('publisher-deleted')}}
    </div>
  </div>
@endif
                </h1>
...
<div style="display:none;" id="myDiv">
    <div class="height-kategorije pb-[30px] scroll">
            ...
            <div class="flex items-center">
                ...
                <script>
                    document.getElementById('pagination').onchange = function() {
                        window.location = "{{ $publishers->url(1) }}&items=" + this.value;
                    };
                </script>
        ...
        <div class="inline-block min-w-full px-[50px] pt-3 align-middle bg-white rounded-bl-lg rounded-br-lg shadow-dashboard">
            ...
            <button type="submit" class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE] button delete-all-publishers" data-url="">
                <div class="icon">
                    <svg class="top">
                        <use xlink:href="#top">
                    </svg>
        ...
        e.preventDefault();
                    ...
                    <tr class="border-b-[1px] border-[#e4dfdf]">
                        ...
                        <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                            ...
                            style="outline: none;border: none;font-weight: bold;"
                                ...
                                <span style="padding-top: 1px;">Izmijeni izdavača</span>
                            </button>
                        </td>
                        ...
                        <td class="px-3 py-5 leading-4 tracking-wider text-left sakriveno checkme2">
                            <button
                            style="outline: none;border: none;font-weight: bold;"
                            class="flex w-full px-1 text-sm leading-5 text-left text-blue-600 outline-none delete-all-publishers" 
                            role="menuitem"
                            type="submit"
                            data-url="">
                            <i class="fas fa-trash mr-[5px] ml-[5px] py-1"></i>
                            <span style="padding-top: 1px;">Izbriši izdavača</span>
                            </button>
                            </td>
                ...
                <tbody class="bg-white" id="tablex">
                    ...
                    <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                        ...
                        <td class="flex flex-row items-center px-4 py-4">
                            <p class="no-select">{{$publisher->name}}</p>
                        </td>
                            ...
                            <div
                                ...
                                <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none"
                                    ...
                                    <div class="py-1">
                                        ...
                                        <button 
                                        data-id="{{ $publisher->id }}" 
                                        data-action="{{ route('publishers.destroy', $publisher->id) }}"onclick="deleteConfirmation({{$publisher->id}})"
            ...
            <div class="m-3">{!! $publishers->links() !!}</div>
    ...
    @else 
        ...
        <div class="w-[400px] flex items-center px-6 py-4 my-4 text-lg bg-[#3f51b5] rounded-lg">                       
            ...
            <p class="font-medium text-white">Trenutno nema izdavača u bazi podataka! </p>
        </div>
    ...
    function deleteConfirmation(id) {
        swal({
            ...
            if (e.value === true) {
                ...
                swal(
                    'Uspješno!',
                    'Izdavač je uspješno izbrisan.',
                    'success'
                    ).then(function() {
                     window.location.href = "/podesavanja/izdavaci";
                    });
                $.ajax({
                    type: 'POST',
                    url: "{{url('/podesavanja/izdavaci')}}/" + id,
...
// Script for edit publisher profile top header
function editPublisher() {
var name = $('#check:checked').val();
window.location.href = "/podesavanja/izmijeni-izdavaca/" + name;
}
</script>
...
<script type="text/javascript">
   ...
   $('.delete-all-publishers').on('click', function(e) {
   var idsArr = [];  
   ...
   $.ajax({
   url: "{{ route('delete-all-publishers') }}",
   type: 'DELETE',
