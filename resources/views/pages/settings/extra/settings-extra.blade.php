        ...
        <div class="height-ucenikProfile pb-[30px] scroll">
            ...
            <div class="section-">
                <div class="flex flex-col">
                    <a href="{{route('readme')}}" class="ml-[30px] mt-[10px] "
                       style="font-size: 20px">
                        <span id="readme">{{__('Pročitajte uputstvo')}}</span>
                        <i style="cursor: default;color: #4558BE"
                           class="fas fa-book-reader"></i>
                    </a>
                    <hr class="mt-[10px]">
                    <h2 class="ml-[30px] mt-[10px]" style="font-size: 25px">
                        {{__('Autori')}}
                        @error('csv_author')
                        <span style="color: red;font-size: 20px"
                              class="mt-[10px]">| {{$message}}</span>
                        @enderror</h2>
                    <div class="pl-[30px] py-[20px] flex border-b-[1px] border-[#e4dfdf]">
                        <div class="file-drop-area @error('csv_author') error-border @enderror">
                            <form action="{{ route('csvAuthors') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf @honeypot
                                @error('csv_author') <i
                                        class="fas fa-exclamation mr-[5px]"
                                        id="icon"></i> @enderror
                    ...
                    <div class="pl-[30px] py-[20px] flex border-b-[1px] border-[#e4dfdf]">
                        <div class="file-drop-area @error('csv_book') error-border @enderror">
                            <form action="{{ route('csvBooks') }}" method="POST"
                    ...
                    <div class="pl-[30px] py-[20px] flex border-b-[1px] border-[#e4dfdf]">
                        <div class="file-drop-area @error('csv_book_author') error-border @enderror">
                            <form action="{{ route('csvBookAuthors') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf @honeypot
                                @error('csv_book_author') <i
                                        class="fas fa-exclamation mr-[5px]"
                                        id="icon"></i> @enderror
                                <span class="fake-btn">{{__('Dodajte CSV fajl')}}</span>
                                <span class="file-msg">{{__('ili prevucite Vaš fajl ovdje')}}</span>
                                <input class="file-input" type="file" multiple
                                       name="csv_book_author">
                        </div>
                        <button style="margin-left: 20px;outline:none;"
                                class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE]">
                            <i class="fas fa-database"></i>&nbsp; {{__('Popunite bazu')}}
                    ...
                    <div class="pl-[30px] py-[20px] flex border-b-[1px] border-[#e4dfdf]">
                        <div class="file-drop-area @error('csv_book_category') error-border @enderror">
                            <form action="{{ route('csvBookCategories') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf @honeypot
                                @error('csv_book_category') <i
                                        class="fas fa-exclamation mr-[5px]"
                                        id="icon"></i> @enderror
                                <span class="fake-btn">{{__('Dodajte CSV fajl')}}</span>
                                <span class="file-msg">{{__('ili prevucite Vaš fajl ovdje')}}</span>
                                <input class="file-input" type="file" multiple
                                       name="csv_book_category">
                        </div>
                        <button style="margin-left: 20px;outline:none;"
                                class="btn-animation inline-flex items-center text-sm py-2.5 px-5 rounded-[5px] tracking-wider text-white bg-[#3f51b5] hover:bg-[#4558BE]">
                            <i class="fas fa-database"></i>&nbsp; {{__('Popunite bazu')}}
                    ...
                    <div class="pl-[30px] py-[20px] flex border-b-[1px] border-[#e4dfdf]">
                        <div class="file-drop-area @error('csv_book_genre') error-border @enderror">
                            <form action="{{ route('csvBookGenres') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf @honeypot
                                @error('csv_book_genre') <i
                                        class="fas fa-exclamation mr-[5px]"
                                        id="icon"></i> @enderror
                                <span class="fake-btn">{{__('Dodajte CSV fajl')}}</span>
                                <span class="file-msg">{{__('ili prevucite Vaš fajl ovdje')}}</span>
